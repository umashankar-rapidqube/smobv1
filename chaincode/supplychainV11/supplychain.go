...
func (t *SimpleChaincode) readAllRequest(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	var startKey
		...
		if len(args) < 2 {
			...
			return shim.Error("Expected two arguments for  readAllRequest")
		}
		if startKey==args[0]{
	...
	startKey = args[0]
		fmt.Println("Reading the request data for startKey:--- ", startKey)
		
		var endKey 	 = args[1]
		fmt.Println("Reading the request data for startKey:--- ", endKey)
		...
		//readAsBytes, _ := APIstub.GetStateByRange(startKey, endKey)
		AllreqAsBytes, err := APIstub.GetStateByRange(args[0],args[0])
		if err != nil {
			return shim.Error(err.Error())
		}
		defer AllreqAsBytes.Close()
		
		//json.Unmarshal(reqAsBytes, &request)
		return shim.Success(AllreqAsBytes)
	}
